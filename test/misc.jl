...
stack, frame = @make_stack map(x->2x, 1:10)
state = dummy_state(stack, frame)
...
stack, frame = @make_stack complicated_keyword_stuff(1)
state = dummy_state(stack, frame)
...
@test runframe(JuliaInterpreter.enter_call(complicated_keyword_stuff, 1, 2)) ==
      runframe(@make_stack(complicated_keyword_stuff(1, 2))[2])
@test runframe(JuliaInterpreter.enter_call(complicated_keyword_stuff, 1, 2; x=7, y=33)) ==
      runframe(@make_stack(complicated_keyword_stuff(1, 2; x=7, y=33))[2])
